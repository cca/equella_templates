# JZK, 2010-08-11: deleteTerm() added
# JZK, 2012-11-14: specifies *not* to HTML encode parameters for taxnomy-related functions
# JZK, 2012-11-14: encodes all HTML POSTs as UTF-8
...
STANDARD_INTERFACE = {'url':'services/SoapService41', 'namespace':'http://soap.remoting.web.tle.com'}
...
class EquellaSoap:
	...
	def _call (self, name, args, returns=1, interface = STANDARD_INTERFACE, htmlencode=True):
		
		token = self._loginWithTokenNextCall
		self._loginWithTokenNextCall = None
		
		url = '%s%s' % (self._institutionUrl, interface['url'])
		requestContent = generate_soap_envelope(name, args, interface['namespace'], token, htmlencode)
		responseContent = ''
		
		try:
			response = self._conn.open( urllib2.Request(url, data=requestContent.encode("utf-8"), headers=self._getHeaders()) )
...
class ItemEditingSession:
	...
	### Experimental for now.  It has only been tested on attachments with a type of "item".  It should also work fine on
	...
	# e.g. item.submit ()
	#  if you use sumbit(0) = will force it to skip moderation.
	def submit (self, workflow=1):
		self.parClient._saveItem (self.newDom.toxml (), ('false', 'true') [workflow])
